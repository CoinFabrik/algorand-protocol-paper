...
and such that the sum of its weights ($\sum_{vt\in V} vt.cred.j\ge CommitteeThreshold(soft)$) is bigger than the committee 
threshold. \marginpar{Revisar que este definido y apuntar a esa seccion.}
...
Every time the function $Proposal()$ is called, both input and output are stored
...
\begin{subsection}{HandleVote}\label{ssect:HandleVote}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleVote$}{$Vote \ vt$}
            ...
            \If{$vt.round = r+1 \land (vt.period > 0 \lor vt.step \in \{next_0,\ldots, next_{249}\})$}
            ...
            $(vt.period = p+1 \land vt.step \in \{next_1,\ldots, next_{249}\}) \lor$ \\
            $(vt.period = p \land vt.step \in \{next_1,\ldots, next_{249}\} \land vt.step \notin \{s-1,s, s+1\}) \lor$ \\
            $(vt.period = p-1 \land vt.step \in \{next_1,\ldots, next_{249}\} \land vt.step \notin \{\bar{s}-1, \bar{s},\bar{s}+1\}))$}
        ...
        \For{$Account \ a \in A$}
            ...
            \If{$cred.j > 0$}
                \If{$\exists v=Proposal_{value}(e) $ for some $e\in P | \mathsf{IsCommitable}(v)$}
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, s, v, cred))$
                \ElsIf{$\nexists s_0 > cert | Bundle(r, p - 1, s_0, \bot) \subseteq V \wedge$ \newline 
                ${}$ ${\hskip 2.65cm} \exists s_1 > cert | Bundle(r, p - 1, s_1, \bar{v}) \subseteq V$}
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, s, \bar{v}, cred))$
                \Else
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, s, \bot, cred))$
                \EndIf
            \EndIf
        \EndFor
        \EndFunction
...
Line 2 means the algorithm will append the proposal that is retrieved by calling $Proposal(v)$ and
...
Otherwise returns $\mathsf{Proposal}(\bot)$, defined as the "empty block" (special value where all
...
\begin{algorithm}[H]\label{algo:compute-seed}
    \begin{algorithmic}[1]
    \Function{$ComputeSeedAndProof$}{$b, Address \ I$}
        \If {$p = 0$}
             \State $y \gets mathsf{VRF.Prove}(Ledger[r-\delta_s].Seed, I_{sk})$
             \State $\alpha \gets \mathsf{H}(VRF.ProofToHash(y), I)$
        \EndIf
