...
timer,\marginpar{como funciona? hay un clock y va en ms? 
Porque sino como funcionan los = del event handler}
%La unidad de todas las constantes temporales en lo que escribí estan en segundos.
...
% calculo e'? BT'? L'?
%En este momento BT y L son contextualmente los que hayan resultado del round anterior. Digamos que estan en un estado inicial BT_r, L_r.
...
and either the period is positive $vt.p>0$ or the step is in the open interval $vt.s\in(cert, late)=(2,253)$\marginpar{es int abierto???}
%adp: si, es int abierto

%% reescribir haciendo algunos assumptions sobre el voto para que pase sin tanta chachara.ponele que vt.s=0 o vt.s=1???
...
%adp: claro, si estamos en un vanilla run y estamos llamando al vote handler con un proposal vote (s=0), solo observa (aka
    %agrega el voto a V) y broadcastea ese voto.
...
$\mathsf{StartNewRound}(vt.round + 1)$ and $\mathsf{GarbageCollect}(r_{old}, p)$\marginpar{que  p es este?}
%este es el p de periodo

%agrego esto que por ahi es importante aclarar
After commiting the block, the node makes all changes outlined by it to its state, appending the new block to $L$,
updating affected records in the balance table $BT$, and box states.
...
implement the protocol while the latter simply relay whatever they receive from
the nodes.
