...
\begin{subsection}{Vote handler}\label{ssect:HandleVote}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleVote$}{$Vote \ vt$}
            ...
            (s=0 \land \mathsf{IsEquivocation}(vt)) \lor
...
\begin{subsection}{Bundle handler}\label{ssect:HandleBundle}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleBundle$}{$Bundle \ b$}
            ...
            \If{$\mathsf{VerifyBundle}(b) \land b.r = r \land b.p \geq p-1$}
                \For{$vote \in b$}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{ObserveNewRound}$}{}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{ObserveNewPeriod}$}{}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{IsEquivocation}$}{$Vote vt$}
...
\begin{algorithm}[H]
    ...
    %period and / or round number have been updated already
    %so current values (r,p) are the new ones
...
\noindent \textbf{Description:}\\
%TODO: talk about scenarios. Que pasa si (r+i, 0), que pasa si (r, p+i)
