...
These proposals should be for the same round $vt.r=r$ and $vt.s=2$ or $\ge 2$\marginpar{Which is it?}
...
When the main algorithm receives a broadcasted vote $v$, as those sent by the $\matshf{HandleProposal}$
algorithm, and none of the time events has kicked in (e.g., because $time<\mathsf{FilterTimeout}$)
it calls the vote handler with $v$ as input: $\mathsf{HandleVote}(v)$.
...
Since this is a {\em certification step} $vt.s\ge cert=2$\marginpar{check}
...
it (calling $\mathsf{RequestProposap}(v)$)\marginpar{es request(v) o request(Proposal(v)???)}
