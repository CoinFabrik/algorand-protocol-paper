...
propose the assembled block $B$, it returns the sortition hash 
...
keeps the one with the lowest observed hash (maximum priority) always
...
any ammount of potentially conflicting proposed blocks in the stage prior into a binary 
choice, either a block proposed by a user or an empty one.
In the first sub-step, each commitee member gossips the proposed block, and then does a 
    ...
    \Function{$BLOCK\_STEP$}{$hblock, step, blockHash$}
        ...
        \ElsIf{$r \neq emptyHash$}
            \For{$step < s^\prime\leq step + 3$}
                \State $CommitteeVote(ctx, round, s^\prime, \tau step, r)$
            \EndFor
            \If {$step = 1$}
...
Since sortition hashes are random, even if an attacker happened to be a committee member with the 
lowest observable hash,
the coin value's randomness would still be preserved, and in successive steps the probability of 
them being chosen again and able to manipulate this stage would severely diminish.
