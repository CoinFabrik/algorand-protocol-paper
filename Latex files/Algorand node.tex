...
When a participation node is booted up, it connects to 4 relay nodes (on average), using
...
DELTA-BALANCE-LOOKBACK rounds after the key registration transaction is approved.
...
TODO: include some voting stats?
...
A Ledger's state is defined by a tuple of 5 components, those being:
...

A Ledger's state is completed by a Balance table, $BT$, a mapping of addresses to balance records.
A Balance record is comprised of a 64 bit unsigned integer raw balance, a 3-state flag status (can be 
"online", "offline", or "non-participating"), its voting keys (more info on the keys subsection) and 
...
For the purpose of this report, transactions are an otherwise opaque object.
...
A vote $vt$, constructed as $Vote(I,r,p,s,v)$ is a tuple with the following members:
\begin{itemize}
...
All votes are broadcast wrapped in a structure called UnauthenticatedVote, where we add
a Credential field, that includes the voter's signature and VRF proof.
As part of the verification process, a node authenticates the received vote and obtains
the vote weight (result of running the sortition procedure) and VRF hash.

Consider now a pair of votes, $vt_0$ and $vt_1$, constructed like
$vt_0 = Vote(I, r, p, s, v_0)$ and $vt_1 = Vote(I, r, p, s, v_1)$, where all values are equivalent
...
\begin{algorithm}[H]
    ...
    \If{$ev \ is \ TimeoutEvent$}
        ...
        \ElsIf{$time = \max\{4\lambda, \Lambda \}$}
            \State $Recovery(next_0)$
        \ElsIf{$time = \max\{4\lambda, \Lambda \} + 2^{st-3}\lambda + r, st \in [4, 252], r \in [0, 2^{st-3}\lambda]$}
            \State $Recovery(next_{st-3})$
...
The following steps are recovery procedures in case there's no observable consensus. 
$next_{s-3}$ with $s \in [3, 252]$ are recovery steps and the last three ($late$, $redo$ and $down$)
are special "fast" recovery steps.
A period is an execution of a subset of steps, executed in order until one of them achieves 
...
There's 3 kinds of messages: votes, proposal payloads, and bundles.
%describe all messages that can be observed
\\

For convenience, we define two special values: Mu() and Sigma().
Mu() is the highest priority observed proposal-value in the current $(r,p)$ context (lowest hashed according to the function outlined
in the $\mathsf{BlockProposal}$ algorithm), or $\bot$ if no valid proposal vote has been observed by the node.
Sigma() is the sole proposal-value for which a soft bundle has been observed (again, in the current $(r,p)$ context), or $\bot$ if
    ...
    \begin{algorithm}
        ...
        \Function{$\mathsf{BlockProposal}$}{}
            ...
            \For{$a \in A$}
                ...
                \mathsf{Sortition}(a_{sk}, proposal)$
                \If{$j > 0$}
                    \If{$p = 0 \lor \exists s^\prime \textfractionsolidus Bundle(r, p-1, s^\prime, \bot) \subset V$}
                        ...
                        \State $\mathsf{Broadcast}(Vote(I, r, p, proposal, v))$
...
First, on line 2, the node attempts a resynchronization (TODO: link to SUBROUTINE). This only has any effect on periods
$p > 0$.
...
\begin{algorithm}[H]
    ...
    \For{$vote \in V | vote.step = proposal$}
        \State $priorityHash \gets \min_{i \in [0, vote.weight)} \{H(vote.sorthash || i)\}$
...
\begin{subsection}{HandleProposal}\label{ssect:HandleProposal}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleProposal$}{$Proposal \ e$}
            ...
            \If{$\lnot VerifyProposal(e) \lor e \in P$ }
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleVote$}{$Vote \ vt$}
            ...
            \If{$\lnot VerifyVote(vt)$}
                \State $PenalizePeer(SENDER\_PEER(vt))$ \ //optional
            ...
            \ElsIf{$vt.step = soft$}
                ...
                \If{$\exists v | Bundle(vt.round,vt.period,soft,v) \subset V$}
                    ...
                    \For{$a \in A$}
                        ...
                        \mathsf{Sortition}(a_{sk}, cert)$
                        \If{$j > 0$}
                            \State $\mathsf{Broadcast}(Vote(a.I, r, p, cert, v))$
                        \EndIf
                    \EndFor
                \EndIf
            \ElsIf{$vt.step = cert$}
        ...
        \For{$Account \ a \in A$}
            ...
            \If{$j > 0$}
                \If{$\exists v | \mathsf{IsCommitable}(v)$}
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, next_k, v))$
                \ElsIf{$\nexists s_0 > cert | Bundle(r, p - 1, s_0, \bot) \subseteq V \wedge$ \newline 
                ${}$ ${\hskip 2.65cm} \exists s_1 > cert | Bundle(r, p - 1, s_1, \bar{v}) \subseteq V$}
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, next_k, \bar{v}))$
                \Else
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, next_k, \bot))$
                \EndIf
            \EndIf
        \EndFor
    ...
    \begin{algorithm}[H]
        ...
        \For{$Account \ a \in A$}
            \If{$\mathsf{IsCommitable}(v)$}
                ...
                \If{$j > 0$}
                    \State $\mathsf{Broadcast}(Vote(a.I, r, p, late, v))$
                \EndIf
            ...
            \ElsIf{$\nexists s_0 > cert | Bundle(r, p - 1, s_0, \bot) \subseteq V \wedge$ \newline 
                ${}$ ${\hskip 2.1cm} \exists s_1 > cert | Bundle(r, p - 1, s_1, \bar{v}) \subseteq V$}
                    ...
                    \If{$j > 0$}
                        \State $\mathsf{Broadcast}(Vote(r,p,redo,\bar{v}))$
                    \EndIf
            ...
            \Else
                ...
                \If{$j > 0$}
                    \State $\mathsf{Broadcast}(Vote(r,p,down,\bot))$
                \EndIf
            \EndIf
        \EndFor
...
\begin{algorithm}[H]
    ...
    \Function{$\mathsf{Sortition}$}{$Account \ a, uint64 \ step$}
        \State $ \langle hash, \pi \rangle \gets \mathsf{VRF}(seed||step)$
