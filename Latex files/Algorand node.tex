...
\end{algorithm}\marginpar{1. Que hace la linea 9. No entiendo. 2. que es ctx ? 3. Cual es 
la estructura del FullBlockToPropose, e.g, (B, seed,...)}
...
$\mathsf{sortitionW}()$ described in Section \ref{sect:soubroutines}).\marginpar{Ojo, no todos estan. Revisar!} 
...
choice, either a block proposed by a user or an empty one.\marginpar{Que es reduction one/two, $\lambda_{step}$, hblock}
...
\begin{itemize}
    ...
    \marginpar{Entonces especifiquemoslo arriba. Porque no aparece}
  \end{itemize}

\noindent \textbf{Description:}\\
In the first step and with the input block $block\; hash$ as input, together
with other parameters, the node does a\marginpar{Ven solo el hash y votan a partir del hash?}
...
amount of votes set by the threshold for this step, or the hash of the default empty\marginpar{cual threshold? que es ese if hblock=timeout?} 
...
\begin{subsection}{Vote Certification}\label{ssect:certifyvote}
    \begin{algorithm}[H]
        \begin{algorithmic}[H]
        \Function{$\mathsf{CertifyVote}$}{$hblock$}
...
\noindent \textbf{Description:}\\
The certify vote stage is the core of the BA* algorithm.\marginpar{What is the BA* algorithm? Introduce it earlier}
The main loop executes a maximum of 254 times (from step 2 to step 255, labeled ``cert'' and ``down'' 
respectively on the specs (\cite{specs})).\marginpar{What labels?}
...
\begin{enumerate}
    ...
    but no block has quite enough votes to push the threshold.\marginpar{No se entiende lo de hangforever. Que quiere decir?}
\end{enumerate}
...
\begin{algorithm}[H]
    ...
    \State $counts \gets \{\}$ \mathtt{// hash table, new keys mapped to 0}
    \State $voters \gets \{\}$
...
\begin{algorithm}[H]
    ...
    \mathtt{// only committee members originate a message}
    \If{$j > 0$}
        \State $\mathsf{GOSSIP\_MSG}(user.pk, Signed_{user.sk} (round,step,sorthash,\pi,\mathsf{Hash}(ctx.last_{block}),value))$
    \EndIf
    \EndFunction
    \end{algorithmic}
...
\begin{algorithm}[H]
    \begin{algorithmic}[H]
    \Function{$\mathsf{ComputeSeed}$}{$ctx, round, B$}
        \If{$B \neq empty\_block$} 
            \State $\Return$ $\mathsf{VRF}_{\mathsf{get_{SKu}}(ctx, r)}(ctx.LastBlock.seed||r$)
        \Else
            \State $\Return$ $\mathsf{H}(ctx.LastBlock.seed||r)$
        \EndIf
    \EndFunction
    \end{algorithmic}
...
The $\mathsf{get}_{SKu}(ctx, round)$ helper function gets the relevant user's secret key 
(according to the signing scheme described in specs).
That is, the secret key from a round b time before block $r-1-(r\text{ mod }R)$, where $R$ 
is the sortition seed's renewal rate, $r$ is the current round's number,
and $b$ is the upper bound for the loss of strong synchrony (according to the weak 
synchrony assumption, that is, $s < b$).\\
...
to their weight (aka. stake) by returning a j parameter, which indicates the number of times that 
