...
block $e$ (see Section~\ref{sect:avm})\marginpar{que onda, aca tiene un clon del estado que presenta como candidato?}.
% sortition
It next loops over all the accounts that are registered to the node. 
For each account, the node runs $\mathsf{Sortition}(a, proposal)$ (Algo.~\ref{algo:sortition}) 
...
Then, the event handler runs $\mathsf{HandleProposal}(e)$ (Algorithm~\ref{algo:handle-proposal}).
The $\mathsf{HandleProposal}$ algorithm receives a proposal $e$ and unpacks its contents,
...
When the main algorithm receives a broadcasted vote $vt$, as those sent by the $\mathsf{HandleProposal}$
...
Let $V*$ be all proposal votes received. By a priority hash function, this stage performs a filtering action,
...
\begin{subsection}{HandleProposal}\label{ssect:handle-proposal}
    ...
    \begin{algorithm}[H]\label{algo:handle-proposal}
        \begin{algorithmic}[1]
