...
\begin{subsection}{HandleVote}\label{ssect:HandleVote}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleVote$}{$Vote \ vt$}
            ...
            \If{$vt.step = 0 \land (vt \in V \lor IsEquivocation(vt))$}
                \State $return$ \ //ignore vote, equivocation not allowed in proposal votes
            \EndIf

            \If{$vt.step > 0 \land IsSecondEquivocation(vt) $}
            ...
            \ElsIf{$vt.step = cert$}
                ...
                \If{$\exists v | Bundle(vt.round,vt.period,cert,v) \subset V$}
                    ...
                    \EndIf
                    \State $Commit(v)$  %ver que pasa si no se pudiera commitear
                    \State $StartNewRound(vt.round)$
                    \State $GarbageCollect()$
                \EndIf
            \Else
                \If{$\exists v,s | Bundle(vt.round,vt.period,s,v) \subset V$}
...
\begin{subsection}{HandleBundle}\label{ssect:HandleBundle}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$HandleBundle$}{$Bundle \ b$}
            ...
            \If{$VerifyBundle(b) \land b.r = r \land b.p + 1 \geq p$}
                \For{$vote \in b$}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{VerifyProposal}$}{$Proposal \ p$}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{StartNewRound}$}{$uint64 \ finishedRound$}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{StartNewPeriod}$}{$uint64 \ finishedPeriod$}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{PenalizeVoter}$}{$Account \ a$}
