...
% block assembly
Next, the $\mathsf{BlockProposal}$ algorithm runs runs. It will ignore the 
...
no longer participate in this round.
When it is nonzero, the node participates in the voting representing the account.

If either the period is zero ($p=0$) or the node received a bundle $\matshf{Bundle}(r,p-1,s,\bot)$
for some number of steps $s\in\Z_{\ge 0}$, then
the algorithm sets $v$ as the proposal value obtained from the block $e$
and does two separate broadcasts for $Vote(a.i, r,p, proposal, v, cred)$ and $e$.
Else, it will broadcast $Vote(a.I, r, p, proposal, \bar v, cred)$\marginpar{De donde sale $\bar v$?}
and every $Proposal(\bar v)$ for $Proposal(\bar v)\in P$.\marginpar{Que es P?}
...
including the XXX $(r,p,s)$
% que es esto del \sigma(r+1,0)=v??
...
$v$ is committable and the step is smaller than the certification (e.g., $s\le cert$)\marginpar{de donde sale s}
then  for every account $a$ registered to the node it runs $\mathsf{Sortition}(a, cert)$
...
\begin{itemize}
    ...
    \item
    A cryptographic commitment to the block's transaction sequence (a vector commitment).\marginpar{En qu\'e se distinguen los cmt?}
...
A proposal-value is a tuple $v = (I, p, Hash(e), Hash(Encoding(e)))$ where 
$I$ is an address (the “original proposer”), 
$p$ is a period (the “original period”), 
{\bf Definir e}
and
$Hash(\,)$ is a cryptographic hash function (implemented as SHA512/256). 
...
In practice, votes are broadcast wrapped in a structure called $UnauthenticatedVote$, 
