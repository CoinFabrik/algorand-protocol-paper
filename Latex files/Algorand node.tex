...
\begin{algorithm}[H]
    ...
    \Function{$ComputeSeed$}{$ctx, r, B$}
        \If{$B \neq empty\_block$} 
            \State $\Return$ $VRF_{get_{SK_u}(ctx, r)}(ctx.LastBlock.seed||r$)
        \Else
...
The $get_{SKu}(ctx, r)$ helper function gets the relevant user's secret ephemeral keys (according to the signing scheme described in specs, the keys 160 rounds prior to $r$).
