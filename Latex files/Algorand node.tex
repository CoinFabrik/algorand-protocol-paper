...
\begin{algorithm}[H]
    ...
    \For{$vt^\prime \in V \text{ with } vt^\prime.step = proposal$}
        \State $priorityHash \gets \min_{i \in [0, vt^\prime.cred_j)} \{ \mathsf{H}(vt^\prime.cred.sh || i)\}$
    ...
    \Function{$\mathsf{VerifyVote}$}{$Vote \ vt$}
        ...
        \If{$vt.step = 0$}
            ...
            \If{$vt.period = vt.v.p_{orig}$}
                \State $valid \gets valid \land vt.v.I_{orig} = vt_I$
            \EndIf
        \EndIf
...
\begin{algorithm}[H]\label{algo:compute-seed}
    ...
    \Function{$\mathsf{ComputeSeedAndProof}$}{$Account \ a$}
        \If {$p = 0$}
             ...
             \State $\alpha \gets \mathsf{H}(\mathsf{VRF.ProofToHash}(y), a_I)$
        \Else
            \State $y \gets 0$
...
\begin{algorithm}[H]\label{algo:verify-seed}
    ...
    \Function{$\mathsf{VerifySeed}$}{$Q, \pi_{seed}, Address \ I_{proposer}$}
        \State $q_0 \gets \mathsf{L}[r-\delta_s]_{seed}$
            \If {$p = 0$}
                \If {$\neg \mathsf{VRF.Verify}(\pi_{seed}, q_0, I)$}
                    \State $\mathsf{return} \ \mathsf{False}$
                \EndIf
                \State $q_1 \gets \mathsf{H}(\mathsf{VRF.ProofToHash}(\pi)||I)$
            \Else
                \State $q_1 \gets \mathsf{H}(q_0)$
            \EndIf
