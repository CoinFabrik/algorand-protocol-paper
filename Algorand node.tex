...
Actually, the network topology includes two kinds of nodes: a set of whitelisted 
...
are hardcoded into the code of participation nodes.\marginpar{como se identifican?}
Relay nodes form a p2p network and will send all the messages they receive from
participation nodes and other relay nodes to the relay nodes connected to them.\marginpar{Y cuando un CLI manda un msj, no recibe response?} 
...
protocol.
A node will queue (multiplex) both kinds of messages together.\marginpar{meter grafico generado por Gero}

Relay nodes are not only responsible for relaying messages. They must also keep a complete
acopy of the ledger. They do not participate in consensus.
