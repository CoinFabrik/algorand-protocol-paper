...
(protocol version 38)\index{protocol version}
...
Participation nodes receive transactions from clients and may participate in the voting
and block generation process.
Participation nodes are permisionless. Yet, they may connect to one relay node at most, 
and relay node addresses are hardcoded into the code of the participation node (in a list 
which includes public key's hashes paired with IP addresses).

Participation nodes have run more and more complex protocol functionality. 
Basically, they run the following services.
\begin{itemize}
    ...
    \item Ledger and balances (see Sect.~\ref{}).\marginpar{ver bien donde y como explicamos este componente}
...
\begin{subsection}{User Interface and Cryptrographic Algorithms}\label{sect:cliandkeys}
    ...
    Participation keys are used to sign all {\bf consensus messages} (or messages of the consensus type),\index{consensus messages}
...
The {\sf state of a node}\index{state of a node} is defined as follows. 
$$
...
\begin{itemize}
    ...
    \item
    $\mathsf{V}$ is the set of all valid {\sf observed votes}\index{observed vote} and 
    {\sf equivocation votes}\index{equivocation vote} in the current execution state
    ...
    It supports \marginpar{Esto no se entiende bien}
...
\end{itemize}
The 3-tuple of nonnegative integers $(r, p, s)$ are referred to as the {\sf execution state}\index{execution state},
the first 7 values $(r, p, s, V, P, \bar{v}, \bar{s})$ are the 
{\sf consensus state parameters},\index{consensus state parameters},
and $\mathsf{L}$ and $\mathsf{BT}$ are referred to as the {\sf ledger state parameters}.\index{ledger state parameters}
...
\begin{itemize}
    ...
    \item A protocol version\index{protocol version} and update state.\marginpar{What do you mean by update state?}
    ...
    \item A 64-byte seed, source of randomness for the consensus algorithm.\marginpar{Esto es $\pi_\mathrm{seed}$}
...
\begin{itemize}
    ...
    \item
    The upgrade vote, which results in the new upgrade state.\index{upgrade state}\marginpar{que es esto} 
    ...
    must not be more than 25 seconds away from said time.\marginpar{que pasa si t1>t0+25 ?}
    ...
    block also duplicates the reward state of the state it transitions into.\marginpar{que es esto?}
    
    \item
    A pair of cryptographic vector commitments to the block's transaction sequence,\marginpar{como se calcula esto?}
...
%\marginpar{los account records no tienen states!}
...
A Proposal is a tuple $prop = (e, \pi_{seed}, p_{orig}, I_{orig})$, where\index{proposal}
...
$I$ is an address (the ``original proposer''), \index{proposal value}
...
proposal} and is denoted as $\bot$.\index{bottom proposal}
...
We define an auxiliary structure, $credentials$, useful in voting, with the following fields:\index{credential}
\begin{itemize}
...
%In the implementation, votes are broadcast wrapped in a structure called ``Unauthenticated Vote'', 
...
An {\sf equivocation vote}\index{equivocation vote} happens 
if all but the proposal values agree, i.e., $I=I^\prime$, $r=r^\prime$, $p=p^\prime$,
$s=s^\prime$, $v\neq v^\prime$ and $cred=cred^\prime$.\marginpar{Tiene sentido que coincidan los I y los cred??}
...
% bundle
A {\sf bundle}\index{bundle} is a set of votes $\mathsf{Bundle}(r,p,s,v)$ such that for any pair of distinct values 
...
For convenience, we define the {\sf observed bundles}\index{observed bundles} for the current execution state, 
$B$\index{B}, as the set of all of the bundles observed by a node in the current execution state. 
