...
\noindent \textbf{Description:}\marginpar{ver como resuelve el nodo la desconexion o blacklist de un peer}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{VerifySortition}$}{$pk, Q, \tau, round, period, step, w, W$}
        \IfThen{$\neg \mathsf{VerifyVRF}_{pk}(hash, \pi, Q||round||period||step)$}{$\mathsf{return} \ 0$}
        \State $p := \frac{t}{W}$
        \State $j := 0$
