...
% ---------------------------------------------------------------------------------------
\author{Argimiro del Pozo\and Ariel Waissbein}
\date{CoinFabrik\\
\today}
...
the {\sf transaction pool},\index{transaction pool}
...
It also supports an $\mathsf{UpdateTP}(TP,e)$ procedure, which takes a valid payset\index{payset}
...
Selection keys used for committee membership selection and verification.
...
and interprets TEAL, or the Transaction Execution\index{TEAL} 
...
\begin{itemize}
    ...
    \item
    $\mathsf{BT}$ is the current balance table \index{balance table} described below.

    \item
    $\mathsf{TP}$ is the transaction pool for the node.
...
\begin{itemize}
    ...
    \item A protocol version\index{protocol version} and update state.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
    \Function{$\mathsf{FilterTimeout}$}{$uint64 \ p^\prime$}
        \If{$p^\prime = 0$}
...
\begin{algorithm}[H]
    ...
    \State $e_{GenesisID} := \mathsf{PrevBlock()_{GenesisID}}$%\mathsf{L}_{genesisID}$

    \State $e_{GenesisHash} := \mathsf{PrevBlock()_{GenesisHash}}$ %\mathsf{H}(\mathsf{L}[0])$
...
\begin{algorithm}[H]
    ...
    \If{$b_r \neq \mathsf{PrevBlock()}_r+1$} \Return $\mathsf{False}$
    \EndIf \\
...
\noindent \textbf{Description:}\\
Block verification algorithm (field by field validation).
...
\noindent \textbf{Description:}\\
Verifies validity of a proposal payload. The function validates the block itself,
field by field (including the seed with the seed cryptographic proof), for a given
ledger context $L$. Lastly, it performs a cryptographic signature verification for
