...
\begin{algorithm}[H]
    ...
    \Function{$\mathsf{ComputeSeedAndProof}$}{$Address \ I$}
        \If {$p = 0$}
             \State $y := \mathsf{VRF_{prove}}(L[r-\delta_s]_Q, secrets(I)_VRFkey)$
...
\begin{itemize}
    ...
    \item $\pi_{seed}$, the 80 byte proof for the $\mathsf{VRF_prove}(.)$ call that computed $Q$
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
    \Function{$\mathsf{Sortition}$}{$Address \ I, uint64 \ period, uint64 \ step$}
        \State $Q := \mathsf{L}[r - \delta_s]_{Q}$

        \State $\pi := \mathsf{VRF_{prove}}(Q||round||period||step, \mathsf{secrets}(I)_{VRFKey})$
