...
from applications or attached to smart signatures. %review? etc. etc.
...
\begin{algorithm}[H]
    ...
    \Function{$\mathsf{RequestProposal}$}{$Proposal_{value} \ v$}
        \State async request $e \neq \bot \ | \ e \in P \implies RetrieveProposal(v) == e$ from $\mathsf{PEERS}$ 
    \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
    ...
    \Function{$\mathsf{DisconnectFromPeer}$}{$\mathsf{PEER\_ID}$}
        \State Remove $\mathsf{PEER\_ID}$ from available connections
    \EndFunction
    \end{algorithmic}
\end{algorithm}
