...
% Network messages are broadcast through P2P established connections,
% while transactions are broadcast utilizing a websocket protocol.
% Both kinds of messages are multiplexed together afterwards.
...
etc etc.
    ...
    \item An implementation of the consensus algorithm.
...
of the Algorand Virtual Machine (AVM)\index{Algorand Virtual Machine}.\marginpar{el voting y demas esta afuera de la avm?}
...
\begin{subsection}{Keys}
    ...
    Spending keys are 32 byte strings used by accounts to sign and verify transactions
    ...
    Participation keys are used to sign all consensus messages.
    %(generated through the goal addpartkey command).
    All private participation keys are stored as secrets by the node,
    while their public keys are a part of online account records, required for any account to 
    participate in consensus.
    They correspond to the ed25119 eliptic curve cryptographic standard (TODO: CITAR standard).
    They are organized in a tree scheme, as follows.
    Previous to online account registration, the node generates a root participation key, a batch of subkeys
    of size $n$, and a sub-batch of ephemeral keys of size $k$.
    
    where $k$ is a field 
    in the key generation command line interface call named $keyDillution$, and defaults to the square
    root of the round range when not specified 
    (e.g. for a $3e+06$ round range a $1712$ sub-batch of ephemeral keys is generated).
    ...
    In the following work, these three cryptographic functions are abstracted as $\mathsf{VRF_{proove}}(.)$, $\mathsf{VRF_{verify}}(.)$ and $\mathsf{VRF_{generate}}(.)$
    respectively.
...
\begin{itemize}
    ...
    \item
    $V$ is the set of all valid {\sf observed votes}\index{observed votes} and 
    {\sf equivocation votes}\index{equivocation votes} in the current execution state
    ($(r,p,s)$).
    
    \item
    $P$ is the set of all valid {\sf observed proposals}\index{observed proposals}
...
A proposal-value is a tuple $v = (I, p, H(prop), H(Encoding(prop)))$ where 
...
$Proposal_{value}(I, p, prop) := (I, p, H(prop), H(Encoding(prop))$ is the function used
...
to the proposal hash $H(prop)$ of the proposal value $v$.
...
We define the {\sf observed bundles}\index{B} for the current execution state, $B$, as the set of all of the bundles
observed by the node in the current execution state.
\end{subsection}
            ...
            \For{$a \in A$}
                ...
                \If{$cred_j > 0$}
                    ...
                    \Else
                        ...
                        %\State $v := v_0 \textfractionsolidus \exists s, Bundle(r,p-1,s,v_0) \subset V$
...
\begin{algorithm}[H]
    ...
    \For{$vt^\prime \in V \text{ with } vt^\prime_s = proposal$}
...
\begin{subsection}{HandleProposal}\label{ssect:handle-proposal}
    ...
    \begin{algorithm}[H]
        ...
        \Function{$\mathsf{HandleProposal}$}{$Proposal \ prop$}
            ...
            \State $v := Proposal_{value}(prop, prop_{p}, prop_{I})$
...
this proposal back from the network.\marginpar{Este request está en el agoritmo? Línea?}
...
\begin{subsection}{HandleVote}\label{ssect:HandleVote}
    ...
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
        \Function{$\mathsf{HandleVote}$}{$Vote \ vt$}
...
\begin{subsection}{Recovery Attempt}
    ...
    \begin{algorithm}[H]
        ...
        \For{$Account \ a \in A$}
            ...
            \If{$cred_j > 0$}
                \If{$\exists v=Proposal_{value}(prop, prop_p, prop_I) $ for some $prop\in P | \mathsf{IsCommitable}(v)$}
...
\begin{algorithm}[H]
    ...
    %https://github.com/algorand/go-algorand/blob/02b99bb25e2df8ca8c46f0caa8de2947121b4367/data/pools/transactionPool.go#L820
        \State $txn := TP.pop()$
        \If{$\mathsf{Valid}(b_{payset}, txn)$}
            \State $b_{payset} :=  b_{payset}||txn$ \\
        \EndIf
    \EndWhile

    \State $\mathsf{return} \ (b, \pi_{seed})$
...
\begin{algorithm}[H]
    ...
    \State $\mathsf{return} \ (\mathsf{RetrieveProposal}(v) \neq \bot) \land (\mathsf{Bundle}(r,p,soft,v) \subset V$)
...
\begin{algorithm}[H]
    ...
    \Function{$\mathsf{RequestProposal}$}{$Proposal_{value} \ v$}
        \State async request $e \neq \bot |e \in P \implies RetrieveProposal(v) == e$ from $PEERS$ 
    \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
    \caption{\underline{Penalize Peer}}
    \label{algo:penalize-peer}
    \begin{algorithmic}[1]
    \Function{$\mathsf{DisconnectFromPeer}$}{$PEER\_ID$}
        \State Remove $PEER\_ID$ from available connections
    \EndFunction
    \end{algorithmic}
\end{algorithm}
...
$PEER\_ID$ is a libp2p peer ID, a base58 encoded multihash string.
Refer to its official documentation for further details.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
    \Function{$\mathsf{ComputeSeedAndProof}$}{$Account \ a$}
        \If {$p = 0$}
...
\begin{algorithm}[H]
    ...
    \Function{$\mathsf{VerifySeed}$}{$Q, \pi_{seed}, Address \ I_{proposer}$}
        \State $q_0 := \mathsf{L}[r-\delta_s]_{seed}$
            \If {$p = 0$}
                \If {$\neg \mathsf{VRF.Verify}(\pi_{seed}, q_0, I)$}
...
\begin{algorithm}[H]
    \caption{\underline{Get Sortition Seed}}
    \label{algo:get-sortition-seed}
    \begin{algorithmic}[1]
...
\begin{algorithm}[H]\label{algo:get-total-online-stake}
    \begin{algorithmic}[1]
        ...
        \State $sum := 0$
        \For{$br \in BT[r^\prime]$}
            \If {$br_{online} \land br_{VRFkey}$}
                \State $sum := sum + br_{balance}$
            \EndIf
        \EndFor

        \State $\mathsf{Return} \ sum$

        \EndFunction
    \end{algorithmic}
    \caption{\underline{getTotalOnlineStake}}
\end{algorithm}
...
\noindent \textbf{Returns:}
\begin{itemize}
    \item the total stake at play in the relevant round $r^\prime$.
  \end{itemize}
...
\begin{algorithm}[H]
    \begin{algorithmic}[1]
    \Function{$\mathsf{VerifySortition}$}{$I, Q, \pi, round, period, step$}
        \State $I_{VRFKey} := BT[round-\delta_s\delta_r][I]_{VRFKey}$
        ...
        \State $w := BT[round-\delta_s\delta_r][I]_{balance}$
        \State $W := \mathsf{getTotalOnlineStake}(round-\delta_s\delta_r)$
        \State $t := \frac{\tau}{W}$
        \State $j := 0$
        \While{$\frac{h}{2^{\mathsf{log_2}(h)}} \notin [\sum_{k=0}^j\mathsf{B}(k;w,t), \sum_{k=0}^{j+1}\mathsf{B}(k;w,t))$}
            \State $j := j+1$
        \EndWhile
        \Return $j$
    \EndFunction
...
\noindent \textbf{Arguments:}
\begin{itemize}
    \item $I$, a user's public spending key (their address)
...
a thruple representing a consensus context of a node ($round$, $period$ and $step$) and utilizes 
