...
transactions according to the fees they pay and may thus allow to newer transactions and 
drop some of the older ones. Note  however that this is how the reference implementation 
...
\begin{algorithm}[H]
    ...
    //define block header
    \State $e_{r} := r$
    \State $e_{prevHash} := \mathsf{H}(\mathsf{L}[r-1])$
    ...
    \State $e_{GenesisID} := \mathsf{L}_{genesisID}$

    \State $e_{GenesisHash} := \mathsf{H}(\mathsf{L}[0])$
