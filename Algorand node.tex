...
\begin{subsection}{Keys}
    ...
    already exist in the Balance table $\mathsf{BT}$), the VRF public key registered, as well as the public participation
    ...
    a valid round number (that is lesser or equal to $r$), and is able to retrieve secret VRF keys 
    and participation keys (in the latter, from any of the tree levels as needed) for a given user $I$ 
    and a valid round number $r^\prime$. As a precondition it is assumed that in any call to $\mathsf{secrets(.)}$,
    the keys were registered and valid for $I$ in the round $r^\prime$, and $I$ generated them
...
\begin{subsection}{Sortition and seed computation subroutines}\label{sect:sort-seed-subroutines}
...
\mathsf{VRF_{prove}}(in, secrets(I)_{VRFkey}) = \pi \land \mathsf{VRF_{proofToHash}}(\pi) = sh$ \\
